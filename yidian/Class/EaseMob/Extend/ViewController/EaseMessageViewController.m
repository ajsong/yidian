...
- (void)viewDidLoad {
	...
	self.view.backgroundColor = [UIColor colorWithRed:243/255.f green:243/255.f blue:243/255.f alpha:1.f];
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[IMGEASE(@"chat_sender_bg") stretchableImageWithLeftCapWidth:15 topCapHeight:30]];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[IMGEASE(@"chat_receiver_bg") stretchableImageWithLeftCapWidth:15 topCapHeight:30]];
    
    [[EaseBaseMessageCell appearance] setSendMessageVoiceAnimationImages:@[IMGEASE(@"chat_sender_audio_playing_full"), IMGEASE(@"chat_sender_audio_playing_000"), IMGEASE(@"chat_sender_audio_playing_001"), IMGEASE(@"chat_sender_audio_playing_002"), IMGEASE(@"chat_sender_audio_playing_003")]];
    [[EaseBaseMessageCell appearance] setRecvMessageVoiceAnimationImages:@[IMGEASE(@"chat_receiver_audio_playing_full"), IMGEASE(@"chat_receiver_audio_playing000"), IMGEASE(@"chat_receiver_audio_playing001"), IMGEASE(@"chat_receiver_audio_playing002"), IMGEASE(@"chat_receiver_audio_playing003")]];
	...
	[self performSelector:@selector(setupChatToolbarProperty) withObject:nil afterDelay:0];
}

- (void)setupChatToolbarProperty{
	self.chatToolbar.showMoreViewPhotoBtn = _showMoreViewPhotoBtn;
	self.chatToolbar.showMoreViewTakePicBtn = _showMoreViewTakePicBtn;
	self.chatToolbar.showMoreViewLocationBtn = _showMoreViewLocationBtn;
	self.chatToolbar.showMoreViewAudioCallBtn = _showMoreViewAudioCallBtn;
	self.chatToolbar.showMoreViewVideoCallBtn = _showMoreViewVideoCallBtn;
	self.chatToolbar.moreViewOtherBtnImage = _moreViewOtherBtnImage;
}
...
- (void)joinChatroom:(NSString *)chatroomId
{
    ...
    [ProgressHUD show:@"Joining the chatroom"];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (weakSelf) {
                ...
                [ProgressHUD dismiss];
                if (error != nil) {
                    [ProgressHUD showError:[NSString stringWithFormat:@"join chatroom \'%@\' failed", chatroomId]];
                } else {
...
{
    [ProgressHUD show:[NSString stringWithFormat:@"\'%@\'join chatroom\'%@\'", aUsername, aChatroom.chatroomId]];
}

- (void)didReceiveUserLeavedChatroom:(EMChatroom *)aChatroom
                            username:(NSString *)aUsername
{
    [ProgressHUD showWarning:[NSString stringWithFormat:@"\'%@\'leave chatroom\'%@\'", aUsername, aChatroom.chatroomId]];
}
                              ...
                              reason:(EMChatroomBeKickedReason)aReason
{
    ...
    {
        _isKicked = YES;
        [ProgressHUD showWarning:[NSString stringWithFormat:@"be removed from chatroom\'%@\'", aChatroom.chatroomId]];
...
- (void)_downloadMessageAttachments:(EMMessage *)message
{
    ...
    void (^completion)(EMMessage *aMessage, EMError *error) = ^(EMMessage *aMessage, EMError *error) {
        ...
        {
            [ProgressHUD showError:@"thumbnail for failure!"];
        }
    };
...
- (void)_locationMessageCellSelected:(id<IMessageModel>)model
{
    ...
    EaseLocationViewController *locationController = [[EaseLocationViewController alloc] initWithLocation:CLLocationCoordinate2DMake(model.latitude, model.longitude) address:model.address];
...
- (void)_videoMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if ([localPath length] == 0) {
        [ProgressHUD showError:@"video for failure!"];
        return;
    }
    ...
    if (videoBody.thumbnailDownloadStatus == EMDownloadStatusFailed || ![[NSFileManager defaultManager] fileExistsAtPath:videoBody.thumbnailLocalPath]) {
        [ProgressHUD showWarning:@"begin downloading thumbnail image, click later"];
    ...
    if ([imageBody type] == EMMessageBodyTypeImage) {
        if (imageBody.thumbnailDownloadStatus == EMDownloadStatusSuccessed) {
            ...
            [[EMClient sharedClient].chatManager asyncDownloadMessageAttachments:model.message progress:nil completion:^(EMMessage *message, EMError *error) {
                ...
                [ProgressHUD showError:@"image for failure!"];
            }];
        } else {
...
- (void)_audioMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if (downloadStatus == EMDownloadStatusDownloading) {
        [ProgressHUD showWarning:@"downloading voice, click later"];
        return;
    }
...
//            moreMessages = [weakSelf.dataSource messageViewController:weakSelf loadMessageFromTimestamp:timestamp count:count];
		...
		if (isAppend) {
			...
			weakSelf.messsagesSource = [[NSMutableArray alloc]init];
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(isEmotionMessageFormessageViewController:messageModel:)]) {
            ...
            if (flag) {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(emotionURLFormessageViewController:messageModel:)]) {
                    ...
                    if (emotion) {
                        model.image = [UIImage em_animatedGIFNamed:emotion.emotionOriginal];
            ...
            if ([[UIDevice currentDevice].systemVersion doubleValue] >= 9.0f) {
                    ...
                    if (asset) {
                        [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData *data, NSString *uti, UIImageOrientation orientation, NSDictionary *dic){
                            if (data.length > 10 * 1000 * 1000) {
                                [ProgressHUD showWarning:@"图片过大，请选择另一张"];
                                return;
                            }
                            if (data != nil) {
                ...
                [alasset assetForURL:url resultBlock:^(ALAsset *asset) {
                    if (asset) {
                        ...
                        NSUInteger bufferSize = [assetRepresentation getBytes:buffer fromOffset:0.0 length:(unsigned long)[assetRepresentation size] error:nil];
                        NSData* fileData = [NSData dataWithBytesNoCopy:buffer length:bufferSize freeWhenDone:YES];
                        if (fileData.length > 10 * 1000 * 1000) {
                            [ProgressHUD showError:@"图片过大，请选择另一张"];
                            return;
                        }
...
- (void)messageCellSelected:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        ...
        {
            _scrollToBottomWhenAppear = NO;
            [ProgressHUD showSuccess:@"Custom implementation!"];
        }
            break;
        default:
            break;
...
- (void)didSendText:(NSString *)text
{
    if (text && text.length > 0) {
        [self sendTextMessage:text withExt:[self sendMessageOfExt]];
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            [ProgressHUD show:@"The recording time is too short"];
...
- (void)moreViewTakePicAction:(EaseChatBarMoreView *)moreView
{
    ...
    [ProgressHUD showWarning:@"模拟器不能使用照相机"];
...
- (void)moreViewAudioCallAction:(EaseChatBarMoreView *)moreView
{
	...
	model.name = _conversationModel.title;
	model.avatarURL = _conversationModel.avatarURLPath;
    [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_CALL object:@{@"chatter":self.conversation.conversationId, @"type":@NO, @"model":model}];
}
...
- (void)moreViewVideoCallAction:(EaseChatBarMoreView *)moreView
{
	...
	model.name = _conversationModel.title;
	model.avatarURL = _conversationModel.avatarURLPath;
    [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_CALL object:@{@"chatter":self.conversation.conversationId, @"type":@YES, @"model":model}];
}

- (void)moreViewOtherAction:(EaseChatBarMoreView *)moreView
{
	// Hide the keyboard
	[self.chatToolbar endEditing:YES];
	
	[self moreViewOtherAction];
}
...
{
    [self sendLocationMessageLatitude:latitude longitude:longitude andAddress:address withExt:[self sendMessageOfExt]];
}
...
- (void)didReceiveCmdMessages:(NSArray *)aCmdMessages
{
    for (EMMessage *message in aCmdMessages) {
        if ([self.conversation.conversationId isEqualToString:message.conversationId]) {
            [ProgressHUD showSuccess:@"receive cmd message"];
            break;
        }
    }
}
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            ...
            model.avatarImage = IMGEASE(@"user");
...
{
	[self sendImageMessageWithData:imageData withExt:[self sendMessageOfExt]];
}
...
- (void)sendImageMessage:(UIImage *)image{
	[self sendImageMessage:image withExt:[self sendMessageOfExt]];
}
...
{
	[self sendVoiceMessageWithLocalPath:localPath duration:duration withExt:[self sendMessageOfExt]];
}
- (void)sendVoiceMessageWithLocalPath:(NSString *)localPath
							 duration:(NSInteger)duration
							  withExt:(NSDictionary *)ext
...
{
	[self sendVideoMessageWithURL:url withExt:[self sendMessageOfExt]];
}
