...
- (void)setModel:(id<IMessageModel>)model
{
	...
	if (model.avatarURLPath) {
		[self.avatarView em_setImageWithURL:[NSURL URLWithString:model.avatarURLPath] placeholderImage:nil completed:^(UIImage *image, NSError *error, EMSDImageCacheType cacheType, NSURL *imageURL) {
			if (!image) self.avatarView.image = model.avatarImage;
		}];
    }
