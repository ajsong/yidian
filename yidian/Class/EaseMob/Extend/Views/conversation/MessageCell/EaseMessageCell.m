...
+ (void)initialize
{
    ...
    cell.bubbleMargin = UIEdgeInsetsMake(3, 0, 3, 0);
                         ...
                         model:(id<IMessageModel>)model
{
    ...
    [_statusButton setImage:IMGEASE(@"messageSendFail") forState:UIControlStateNormal];
	...
	_hasRead.textColor = [UIColor colorWithRed:204/255.f green:204/255.f blue:204/255.f alpha:1.f];
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (messageType) {
            ...
            case EMMessageBodyTypeImage:
            {
                ...
                _bubbleView.imageView.image = IMGEASE(@"imageDownloadFail");
            }
                break;
            case EMMessageBodyTypeLocation:
            {
                ...
                _bubbleView.locationImageView.image = [IMGEASE(@"chat_location_preview") stretchableImageWithLeftCapWidth:10 topCapHeight:10];
            ...
            case EMMessageBodyTypeVideo:
            {
                ...
                _bubbleView.videoTagView.image = IMGEASE(@"messageVideo");
            }
                break;
        ...
        switch (model.bodyType) {
            ...
            case EMMessageBodyTypeImage:
            {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
						_bubbleView.imageView.image = [UIImage imageNamed:_model.failImageName];
						[_bubbleView.imageView em_setImageWithURL:[NSURL URLWithString:_model.fileURLPath] placeholderImage:nil];
                    } else {
            ...
            case EMMessageBodyTypeVoice:
            {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 && [self.recvMessageVoiceAnimationImages count] > 0) {
                        ...
                        self.bubbleView.voiceImageView.image = self.model.isSender ? IMGEASE(@"chat_sender_audio_playing_full") : IMGEASE(@"chat_receiver_audio_playing_full");
                    }
                }
            ...
            case EMMessageBodyTypeVideo:
            {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
						_bubbleView.videoImageView.image = [UIImage imageNamed:_model.failImageName];
						[_bubbleView.videoImageView em_setImageWithURL:[NSURL URLWithString:_model.fileURLPath] placeholderImage:nil];
                    } else {
