...
- (void)_setupBadgeViewConstraint
{
	...
	[self.constraints addObject:[NSLayoutConstraint constraintWithItem:self.badgeView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:self.badgeView attribute:NSLayoutAttributeWidth multiplier:1.0 constant:0]];
	
	[self.constraints addObject:[NSLayoutConstraint constraintWithItem:self.badgeView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.imageView attribute:NSLayoutAttributeRight multiplier:1.0 constant:-self.imageCornerRadius + 3]];
	[self.constraints addObject:[NSLayoutConstraint constraintWithItem:self.badgeView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.imageView attribute:NSLayoutAttributeTop multiplier:1.0 constant:-3]];
...
- (void)setBadgeSize:(CGFloat)badgeSize
{
    if (_badgeSize != badgeSize) {
        ...
        _badgeView.layer.cornerRadius = _badgeSize / 2;
		if (_badgeSize <= 12) self.badgeFont = [UIFont boldSystemFontOfSize:1];
        [self performSelector:@selector(_updateBadgeViewWidthConstraint) withObject:nil afterDelay:0];
    }
}
