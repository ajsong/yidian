...
- (void)setEmotion:(EaseEmotion *)emotion
{
    ...
    if ([emotion isKindOfClass:[EaseEmotion class]]) {
        ...
        [_imageButton setImageEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
        [_imageButton setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
        [_imageButton setImage:IMGEASE(@"faceDelete") forState:UIControlStateNormal];
        [_imageButton setImage:IMGEASE(@"faceDelete_select") forState:UIControlStateHighlighted];
        [_imageButton addTarget:self action:@selector(sendEmotion:) forControlEvents:UIControlEventTouchUpInside];
    }
}
...
- (id)initWithFrame:(CGRect)frame
{
	...
	if (self) {
		...
		_pageControl.frame = CGRectMake(0, CGRectGetHeight(self.frame), CGRectGetWidth(self.frame), 20);
		_pageControl.hidden = YES;
		_pageControl.currentPage = 0;
		_pageControl.numberOfPages = 1;
		_pageControl.pageIndicatorTintColor = [UIColor colorWithRed:204/255.f green:204/255.f blue:204/255.f alpha:1.f];
		_pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:153/255.f green:153/255.f blue:153/255.f alpha:1.f];
...
{
	if (_emotionManagers == nil || [_emotionManagers count] == 0) return 0;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
	...
	[cell sizeToFit];
	if (_emotionManagers == nil || [_emotionManagers count] == 0) return cell;
...
{
	if (_emotionManagers == nil || [_emotionManagers count] == 0) return CGSizeMake(0, 0);
    EaseEmotionManager *emotionManager = [_emotionManagers objectAtIndex:section];
...
- (void)loadPageControl{
	if (_emotionManagers.count>1) return;
	NSInteger count = [[(EaseEmotionManager*)_emotionManagers.firstObject emotions] count];
	_pageControl.numberOfPages = floor(count/(3*7)) + (fmod(count,3*7) ? 1 : 0);
