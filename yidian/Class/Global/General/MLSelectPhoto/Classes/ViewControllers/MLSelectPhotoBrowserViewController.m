...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        [deleleBtn setImage:[[UIImage imageNamed:MLSelectPhotoSrcName(@"icon_image_yes")] imageWithTintColor:[UIColor grayColor]] forState:UIControlStateNormal];
...
- (void)refreshAsset{
    ...
    if([[self.deleteAssets allValues] count] == 0 || [self.deleteAssets valueForKeyPath:[NSString stringWithFormat:@"%ld",(self.currentPage)]] == nil){
        [self.deleleBtn setImage:[[UIImage imageNamed:MLSelectPhotoSrcName(@"icon_image_no")] imageWithTintColor:[UIColor grayColor]] forState:UIControlStateNormal];
    }else{
...
- (void)deleteAsset{
    ...
    if ([self.doneAssets containsObject:[self.photos objectAtIndex:self.currentPage]]) {
        ...
        [self.deleleBtn setImage:[[UIImage imageNamed:MLSelectPhotoSrcName(@"icon_image_yes") ] imageWithTintColor:[UIColor grayColor]] forState:UIControlStateNormal];
...
- (void)setPageLabelPage:(NSInteger)page{
    self.title = [NSString stringWithFormat:@"%d / %ld", (int)(page + 1), (long)self.photos.count];
}
