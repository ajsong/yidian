...
- (void)setSelectPickerAssets:(NSArray *)selectPickerAssets{
    ...
    self.collectionView.doneAssets = self.selectAssets;
...
- (void)refresh:(NSNotification *)noti{
    ...
    self.collectionView.selectAssets = noti.userInfo[@"assets"];
    self.collectionView.doneAssets = noti.userInfo[@"assets"];
...
- (void) pickerCollectionViewDidSelected:(MLSelectPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(MLSelectPhotoAssets *)deleteAssets{
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        ...
        NSInteger count = 0;
        if (pickerCollectionView.selectAssets.count < self.selectAssets.count) {
            count = pickerCollectionView.selectAssets.count;
        }else{
            count = self.selectAssets.count;
        }
...
- (void)dealloc{
    ...
    // 赋值给上一个控制器
    self.groupVc.selectAssets = self.selectAssets;
}
