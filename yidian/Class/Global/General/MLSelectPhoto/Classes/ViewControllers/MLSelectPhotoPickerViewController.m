...
- (void)setSelectPickers:(NSArray *)selectPickers{
    ...
    self.groupVc.selectAssets = selectPickers;
}
...
- (void) done:(NSNotification *)note{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([self.delegate respondsToSelector:@selector(pickerViewControllerDoneAssets:)]) {
            [self.delegate pickerViewControllerDoneAssets:selectArray];
...
- (void)setDelegate:(id<MLSelectPhotoDelegate>)delegate{
    _delegate = delegate;
