...
- (void)setDataArray:(NSArray *)dataArray{
    ...
    if (self.isRecoderSelectPicker){
        ...
        for (MLSelectPhotoAssets *asset in self.selectAssets) {
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        // 需要记录选中的值的数据
        if (self.isRecoderSelectPicker) {
            for (MLSelectPhotoAssets *asset in self.doneAssets) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:PICKER_PUSH_BROWSERPHOTO object:nil userInfo:@{@"currentPage":@(indexPath.row),@"selectAssets":self.selectAssets}];
}
