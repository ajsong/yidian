...
- (void)drawSlicesSeparators:(CGContextRef)contextRef withViewSize:(CGSize)viewSize andCenter:(CGPoint)center{
	NSInteger outerDiameter = viewSize.width;
    CGFloat outerRadius = outerDiameter / 2 - self.internalPadding;
    NSInteger innerDiameter = outerDiameter - self.theme.thickness;
    CGFloat innerRadius = innerDiameter / 2;
    
    if (! self.theme.sliceDividerHidden) {
        long sliceCount = self.progressTotal;
        CGFloat sliceAngle = (2 * M_PI) / sliceCount;
...
- (void)drawCenter:(CGContextRef)contextRef withViewSize:(CGSize)viewSize andCenter:(CGPoint)center{
	NSInteger innerDiameter = viewSize.width - self.theme.thickness;
    CGFloat innerRadius = innerDiameter / 2;
...
- (void)notifyProgressChange{
	...
	CGFloat percentageCompleted = (100.0f / self.progressTotal) * self.progressCounter;
