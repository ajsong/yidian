...
@interface goodsList ()<UITableViewDataSource,UITableViewDelegate,OutletDelegate>{
	NSMutableArray *_ms;
	UITableView *_table;
	NSInteger _offset;
...
- (void)loadData{
	...
	_offset = 0;
	[Common getApiWithParams:@{@"app":@"goods", @"act":@"search", @"type_id":_type_id} success:^(NSMutableDictionary *json) {
...
- (void)loadMore{
	[Common getApiWithParams:@{@"app":@"goods", @"act":@"search", @"type_id":_type_id, @"offset":@(_offset)} success:^(NSMutableDictionary *json) {
		if ([json[@"data"] isArray]) {
			NSArray *list = json[@"data"];
			list = [list UpyunSuffix:@"!medium" forKeys:@[@"default_pic", @"pic"]];
			for (int i=0; i<list.count; i++) {
