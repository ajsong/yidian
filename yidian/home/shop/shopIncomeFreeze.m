...
@interface shopIncomeFreeze ()<UITableViewDataSource,UITableViewDelegate>{
	NSMutableArray *_ms;
	UITableView *_table;
	NSInteger _offset;
...
@implementation shopIncomeFreeze
...
- (void)loadData{
	...
	_offset = 0;
	[Common getApiWithParams:@{@"app":@"income", @"act":@"freeze"} success:^(NSMutableDictionary *json) {
...
- (void)loadMore{
	[Common getApiWithParams:@{@"app":@"income", @"act":@"freeze", @"offset":@(_offset)} success:^(NSMutableDictionary *json) {
		if ([json[@"data"] isArray]) {
			NSArray *list = json[@"data"];
			for (int i=0; i<list.count; i++) {
				[_ms addObject:list[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	UIView *view = [[UIView alloc]initWithFrame:CGRectMake(15, 10, tableView.width-15*2, 35)];
	...
	NSString *string = STRINGFORMAT(@"<p>%@元</p> (%@)", _ms[row][@"money"], _ms[row][@"status_name"]);
