...
@interface shopIncomeHistory ()<UITableViewDataSource,UITableViewDelegate>{
	NSMutableArray *_ms;
	UITableView *_table;
	NSInteger _offset;
...
@implementation shopIncomeHistory
...
- (void)loadData{
	...
	_offset = 0;
	[Common getApiWithParams:@{@"app":@"withdraw", @"act":@"history"} success:^(NSMutableDictionary *json) {
...
- (void)loadMore{
	[Common getApiWithParams:@{@"app":@"withdraw", @"act":@"history", @"offset":@(_offset)} success:^(NSMutableDictionary *json) {
		if ([json[@"data"] isArray]) {
			NSArray *list = json[@"data"];
			for (int i=0; i<list.count; i++) {
				[_ms addObject:list[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	UIView *view = [[UIView alloc]initWithFrame:CGRectMake(15, 10, tableView.width-15*2, 0)];
	[cell.contentView addSubview:view];
	
	UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, view.width, 15)];
	...
	label = [[UILabel alloc]initWithFrame:CGRectMake(0, label.bottom, view.width, 20)];
	label.text = _ms[row][@"title"];
	label.textColor = BLACK;
	label.font = FONT(13);
	label.backgroundColor = [UIColor clearColor];
	[view addSubview:label];
	
	if ([_ms[row][@"memo"] length]) {
		label = [[UILabel alloc]initWithFrame:CGRectMake(0, label.bottom, view.width, 20)];
		label.text = STRINGFORMAT(@"原因：%@", _ms[row][@"memo"]);
		label.textColor = COLORRGB(@"fa5064");
		label.font = FONT(13);
		label.backgroundColor = [UIColor clearColor];
		[view addSubview:label];
		[label autoHeight];
	...
	NSString *string = STRINGFORMAT(@"<p>%@元</p> (%@)", _ms[row][@"withdraw_money"], _ms[row][@"status_name"]);
	NSDictionary *style = @{@"body":@[FONT(13), BLACK], @"p":COLORRGB(@"fa5064")};
	label = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, view.width, 35)];
	label.attributedText = [string attributedStyle:style];
	label.textAlignment = NSTextAlignmentRight;
	label.backgroundColor = [UIColor clearColor];
	[view addSubview:label];
	
	[_cellHeight replaceObjectAtIndex:row withObject:@(view.bottom+10)];
	return cell;
}
