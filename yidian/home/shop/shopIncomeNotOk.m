...
@interface shopIncomeNotOk ()<UITableViewDataSource,UITableViewDelegate>{
	NSMutableArray *_ms;
	UITableView *_table;
	NSInteger _offset;
...
@implementation shopIncomeNotOk
...
- (void)loadData{
	...
	_offset = 0;
	[Common getApiWithParams:@{@"app":@"income", @"act":@"notok_income"} success:^(NSMutableDictionary *json) {
...
- (void)loadMore{
	[Common getApiWithParams:@{@"app":@"income", @"act":@"notok_income", @"offset":@(_offset)} success:^(NSMutableDictionary *json) {
		if ([json[@"data"] isArray]) {
			NSArray *list = json[@"data"];
			for (int i=0; i<list.count; i++) {
				[_ms addObject:list[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	label.text = STRINGFORMAT(@"%@元　　未结算", _ms[row][@"money"]);
	label.textColor = BLACK;
